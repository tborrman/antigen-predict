...
parser <- ArgumentParser(description="Script to compute cross-reactivity heatmaps and correlation plots on 1YMM pdb")
...
my_corr <- function(abundant_df, score_df) {
  ...
  clean_abundant_df <- abundant_df[-c(5,8)]
  clean_score_df <- score_df[-c(5,8)]
...
for (i in peptide_numbers){
  ...
  heatmap.2(abundant_matrix, Rowv = FALSE, Colv =FALSE, dendrogram = "none", 
            trace = "none",main= paste('Ob.1A12 TCR','\n', 'Top ', i, ' most abundant peptides', sep=""), 
...
pdf (paste(wkdir, "/", args$score, "_corr.pdf", sep=""))
  ...
  plot(peptide_numbers, corrs, main= 'Ob.1A12', cex.main = 1.8, cex.lab= 1.5, xlab= 'Number of peptides', ylim=c(-0.2, 1.0), ylab= "Pearson's r", pch=20, type='o', col='mediumorchid4')
dev.off();
