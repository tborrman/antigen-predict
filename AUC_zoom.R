...
plot_ROC <- function(scores, labels, TCR, path, x) {
  ...
  png(paste(path, "/", TCR, "/AUC/ROC_AUC_", TCR,"_", x, "_reads_cutoff_zoom_0.01.png", sep=""), 
      height=1800, width=1800, res=300)
    plot(perf, col=as.list(mycolors), main=TCR, lwd=2, xlim=c(0, 0.01), ylim=c(0,0.25))
    abline(a=0, b=1, col="black", lty=2)
    auc.perf <- performance(pred, measure = "auc")
    legend("topleft",paste(colnames(scores), rep(": ", 4),
