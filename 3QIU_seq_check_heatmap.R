...
parser <- ArgumentParser(description="Make heatmap to check selection round sequence results for 3QIU against Birnbaum results")
parser$add_argument("-r", help="selection round", type="character", dest="round", required=TRUE)

args <- parser$parse_args()
wkdir <- getwd()
file <- "3QIU_sub_matrix_seq_check"
...
heatmap.2(matrix, Rowv = FALSE, Colv =FALSE, dendrogram = "none", 
          trace = "none", main=paste("226 TCR: ", args$round, " library", sep=""), xlab = "Peptide Position", ylab = "Amino Acid", 
