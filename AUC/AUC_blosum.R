...
plot_ROC <- function(scores, labels, TCR, path, x) {
  ...
  mycolors <- c("orange", "magenta", "dodgerblue", "red", "green3")
  pred <- prediction(scores, labels)
  perf <- performance(pred,"tpr","fpr")
  png(paste(path, "/", TCR, "/AUC/ROC_AUC_", TCR,"_", x, "_reads_cutoff_blosum.png", sep=""), 
      height=1800, width=1800, res=300)
    plot(perf, col=as.list(mycolors), main=TCR, lwd=2)
    abline(a=0, b=1, col="black", lty=2)
    auc.perf <- performance(pred, measure = "auc")
    legend("bottomright",paste(colnames(scores), rep(": ", 5),
...
plot_PR <- function(scores, labels, TCR, path, x) {
  ...
  png(paste(path, "/", TCR, "/AUC/PR_AUC_", TCR,"_", x, "_reads_cutoff_blosum.png", sep=""), 
      height=1800, width=1800, res=300)
  plot(perf, col=as.list(mycolors), main=TCR, lwd=2)
  #abline(a=0, b=1, col="black", lty=2)
  auc.perf <- performance(pred, measure = "aucpr")
  legend("topright",paste(colnames(scores), rep(": ", 5),
                             round(as.numeric(auc.perf@y.values),2), sep=""), 
         col=mycolors, title="AUC", lty=1, lwd=2)
  dev.off()
  return()
...
df_lab <- data.frame(df$labels, df$labels, df$labels, df$labels, df$labels)
