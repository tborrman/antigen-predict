...
plot_ROC <- function(scores, labels, TCR, path, x) {
  ...
  #   x: numeric percentile cutoff for read counts to identify as a positive
  mycolors <- c("orange", "magenta", "dodgerblue", "black", "red", "green3")
  pred <- prediction(scores, labels)
  perf <- performance(pred,"tpr","fpr")
  png(paste(path, "/", TCR, "/AUC/ROC_AUC_", TCR,"_", x, "_percentile_cutoff_all_scores.png", sep=""), 
      height=1800, width=1800, res=300)
    plot(perf, col=as.list(mycolors), main=TCR, lwd=2)
    abline(a=0, b=1, col="black", lty=2)
    auc.perf <- performance(pred, measure = "auc")
    legend("bottomright",paste(colnames(scores), rep(": ", 6),
...
plot_PR <- function(scores, labels, TCR, path, x) {
  ...
  png(paste(path, "/", TCR, "/AUC/PR_AUC_", TCR,"_", x, "_percentile_cutoff_all_scores.png", sep=""), 
      height=1800, width=1800, res=300)
...
if (args$t == "1YMM") {
  ...
  negatives <- read.table(paste(wkdir, "/", args$t, 
                                "/AUC/score_table_hamming_blosum_ADLIAYLKQATKG_preselection_remove_rnd4.txt",
                                sep=""), header=TRUE, sep="\t")
  positives <- read.table(paste(wkdir, "/", args$t,
                                "/AUC/score_table_hamming_blosum_ADLIAYLKQATKG_round4.txt",
                                sep=""), header=TRUE, sep="\t")
}
...
scores <- df[c("standard", "bind", "bind_pep_alone", "atlas", "hamming")]
...
df_lab <- data.frame(df$labels, df$labels, df$labels, df$labels, df$labels, df$labels)
