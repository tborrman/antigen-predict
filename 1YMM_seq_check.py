...
def main():
	parser=argparse.ArgumentParser(description='Build substitution matrix for 1YMM selections output by 1YMM_process_step1.py to check against Birnbaum results')
	...
	sub_matrix = np.zeros((20,14), dtype=float)
	...
	for seq_record in SeqIO.parse(fastq, 'fastq', IUPAC.ambiguous_dna):
		...
		# Counted peptide position in codon seq by hand (37-78) 0-based inclusive
		codon_pep = seq_record.seq[37:79]
	...
	np.savetxt('1YMM_sub_matrix_seq_check.txt', percent_matrix, delimiter='\t', fmt='%1.4f')
