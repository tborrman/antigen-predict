class PDB:
	...
	def renumber_ATOM(self):
		'''
		Renumber ATOM field from 1..N 
		with N total ATOMS
		'''
		OUT = open(self.ID + '.renumberATOM.pdb', 'w')
		atom_cnt = 0

		for line in self.FH:
			splitline = line.split()
			atomline = splitline[0]
			if atomline == 'ATOM':
				atom_cnt += 1
				OUT.write(atomline + '{:7d}'.format(atom_cnt) + line[11:])
			else:
				OUT.write(line)
		OUT.close()
		return
